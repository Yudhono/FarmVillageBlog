...
<section class="blog-w3ls">
	<div class="container">
		<h3 class="text-center slideanim">News related to Tag: {{$tak->name}}</h3>
						...
						<div class="post">
							<a href="#">
								<div class="hover01 column">
									<div>
										<figure><img src="{{asset('/images/' . $tautan->image)}}" class="img-responsive" height="auto" width="auto" /></figure>
									</div>
								</div>
							</a>

								<h3 class="ctitle">{{ $tautan->title }}</h3>
								<p class="blog-p1" style="margin-top: 6px"><small>Published: {{ date('M j, Y', strtotime($tautan->created_at)) }}</small></p>

								<p class="blog-p2" style="margin-top: 6px">{{ substr(strip_tags($tautan->body), 0, 300) }}{{ strlen(strip_tags($tautan->body)) > 300 ? "..." : "" }}</p>
								<p class="blog-p3"><a href="{{ url('blog/'.$tautan->slug) }}" class="btn-outline" style="margin-top: 6px">Read More</a></p>
						</div>
